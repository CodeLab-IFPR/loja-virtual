...
<section class="w-full p-6 bg-[#D9D9D9] border border-black rounded-2xl shadow-none space-y-6">
    ...
    <x-modal name="confirm-user-deletion" :show="$errors->userDeletion->isNotEmpty()" focusable>
        <form method="post" action="{{ route('profile.destroy') }}" class="p-6 bg-[#D9D9D9] border border-black rounded-2xl shadow-none">
            @csrf
            @method('delete')

            <h2 class="text-xl font-normal text-gray-800 uppercase text-center" style="font-family: 'Average Sans', sans-serif;">
            ...
            <div class="mt-6">
                ...
                <div class="relative max-w-xs mx-auto">
                    ...
                    <x-text-input
                        ...
                        name="password"
                        type="password"
                        class="block w-full pl-10 py-3 bg-[#D9D9D9] border border-black rounded-md placeholder-black focus:border-black focus:ring-black"
                        placeholder="{{ __('Senha') }}"
                        autocomplete="current-password"
                    />
                </div>

                <x-input-error :messages="$errors->userDeletion->get('password')" class="mt-2 text-sm text-red-600 text-center" />
            </div>
