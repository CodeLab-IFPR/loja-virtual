        ...
        <div class="flex flex-col lg:flex-row gap-8">
            ...
            <div class="lg:w-1/4">
                <div class="bg-white rounded-lg shadow-sm p-6 mb-6">
                    ...
                    <form id="filterForm" method="GET" action="{{ route('catalog') }}" class="max-h-96 overflow-y-auto pr-3  -mr-4">                        
                        <!-- Search -->
                        <div class="mb-6">
                            ...
                            <div>
                                @foreach($categories as $category)
                                    <label class="flex items-center text-sm cursor-pointer hover:bg-gray-50 p-1 rounded">
                                        ...
                                        <span class="ml-auto text-xs text-gray-400">({{ $category->activeProductsCount() }})</span>
                                    </label>
                                @endforeach
                            </div>
                        </div>
                            ...
                            <div>
                                @foreach($materials as $material)
                                    <label class="flex items-center text-sm cursor-pointer hover:bg-gray-50 p-1 rounded">
                                        ...
                                        <span class="ml-auto text-xs text-gray-400">({{ $material->activeProductsCount() }})</span>
                                    </label>
                                @endforeach
                            </div>
                        </div>
                            ...
                            <div>
                                @foreach($colors as $color)
                                    <label class="flex items-center text-sm cursor-pointer hover:bg-gray-50 p-1 rounded">
                                        ...
                                        <span class="ml-auto text-xs text-gray-400">({{ $color->activeProductsCount() }})</span>
                                    </label>
                                @endforeach
                            </div>
                        </div>
                        ...
                        <div class="flex flex-wrap items-center gap-2">
                            ...
                            @if(request('search'))
                                <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-green-100 text-green-800">
                                    ...
                                    <button onclick="clearFilter('search')" class="ml-1 text-green-600 hover:text-green-800">×</button>
                                </span>
                            @endif
                            ...
                            @if(request('categories'))
                                @foreach($categories->whereIn('id', request('categories')) as $category)
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-blue-100 text-blue-800">
                                        {{ $category->name }}
                                        <button onclick="clearCategoryFilter({{ $category->id }})" class="ml-1 text-blue-600 hover:text-blue-800">×</button>
                                    </span>
                                @endforeach
                            @endif
                            ...
                            @if(request('price_ranges'))
                                @foreach(request('price_ranges') as $range)
                                    ...
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-yellow-100 text-yellow-800">
                                        ...
                                        <button onclick="clearPriceFilter('{{ $range }}')" class="ml-1 text-yellow-600 hover:text-yellow-800">×</button>
                                    </span>
                                @endforeach
                            @endif
                            ...
                            @if(request('colors'))
                                @foreach($colors->whereIn('id', request('colors')) as $color)
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
                                        {{ $color->name }}
                                        <button onclick="clearColorFilter({{ $color->id }})" class="ml-1 text-purple-600 hover:text-purple-800">×</button>
                                    </span>
                                @endforeach
                            @endif
                            ...
                            @if(request('dimensions'))
                                @foreach(request('dimensions') as $dimension)
                                    <span class="inline-flex items-center px-3 py-1 rounded-full text-xs font-medium bg-gray-100 text-gray-800">
                                        {{ $dimension }}
                                        <button onclick="clearDimensionFilter('{{ $dimension }}')" class="ml-1 text-gray-600 hover:text-gray-800">×</button>
                                    </span>
                                @endforeach
                            @endif
